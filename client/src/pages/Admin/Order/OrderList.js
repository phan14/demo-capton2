  ...
  return (
    <Row>
          ...
          {showModalUpdate && orderDetail && orderDetail?.orderStatus?.text && (
            <div>
              ...
              {orderDetail?.orderStatus?.code < steps?.length - 1 && (
                <Button variant="success" disabled={loadingUpdate} className="mt-4 d-flex" style={{ margin: "0 auto", background: "red" }} onClick={handleCallApiChangeStatus}>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Oder <Badge bg="secondary">{orderDetail?._id}</Badge>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
                      ...
                      <tr key={item?._id}>
                        ...
                        <td>
                          ...
                          {moment(item.createdAt).isSame(moment(), 'day') && (
                            <span style={{ backgroundColor: "red" }} className="badge">{moment(item?.createdAt).fromNow()}</span>
                          )}
                        </td>
